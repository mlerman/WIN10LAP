...
if(file_exists("c:/UniServer/www/doc/files".$link_with_run)){
	echo "exists !<br/>\n"; 
	$icon_with="<a href='/doc/files/common/download".$linuxstr."file.php?fname=ui_run.run&targetdir=".$link_with_dir."&admin=0'><img src='/doc/images/Play-1-Hot-icon.png'/></a>";
}
...
$link_with="<a href='/doc/files/common/download".$linuxstr."file.php?fname=".$commanderstr."&targetdir=C:".$parent_dir."&urldir=".$link_with_dir_url."&perma=C:/UniServer/www/doc/files/common/perma".$linuxstr."'><img src='/doc/images/".$commanderpng."' /></a>&nbsp;".'<a href="/doc/files'.$_GET['linkwith'].'" target="'.$with_name.'">'.$with_name.'</a>'.$icon_with."<br/>\n";
echo "link_with=".$link_with;

$file_with='c:/UniServer/www/doc/files'.substr($_GET['linkwith'],0,-29).'.next';
$fsize = filesize($file_with);
// ok but only if there no other link in it
// we tolarate only one next
if($fsize) {
  echo "<br/>Not allowed. A project can have only one 'next project'. <br/>Next of ".$with_name." is not empty.<br/>Close this tab.";
  exit(0);
}
  $file_parent='c:/UniServer/www/doc/files'.$_GET['parent'].'.previous';

  if( strpos(file_get_contents($file_parent),$link_with) === false) {
    echo file_put_contents($file_parent, $link_with/*." prev_parent"*/);
  ...
  //echo $file_with;
  // put a next link in the previous project
  if( strpos(file_get_contents($file_with),$link_parent) === false) {
    echo file_put_contents($file_with, $link_parent/*." prev_with_append"*/, FILE_APPEND);
