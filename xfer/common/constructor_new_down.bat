...
if exist ..\_constructor\copy_over_those_files\*.* ( echo copying directory copy_over_those_files
xcopy ..\_constructor\copy_over_those_files\*.* %2\ /Y /s
...
rem adding the project in the related section
echo ^<a href=^"%THISPLACE%/%2/open-command-prompt-here.html^" target=^"%2^"^>%2^</a^>^<br/^> >>.related
echo ^<a href=^"%THISPLACE%/open-command-prompt-here.html^" target=^"%currentfolder%^"^>%currentfolder%^</a^>^<br/^> >>%2\.related
rem pause
echo HOST=%HOST%
echo URLDIR=%URLDIR%
set udir=%URLDIR:~5%
...
rem pause
start C:\"Program Files (x86)"\Google\Chrome\Application\chrome.exe http://%HOST%%THISPLACE%/%2/open-command-prompt-here.html
